...
const ignore = require('ignore') as any;
import { confirm } from '@inquirer/prompts';
import { ToolContext, ToolDefinition, ToolResult } from '../types.js';
...
async function approval(ctx: ToolContext, label: string, detail: string): Promise<boolean> {
...
async function ignored(root: string) {
  ...
  if (await exists(gi)) ig.add(await fs.readFile(gi, 'utf8'));
  return ig;
}
...
async function listFiles(ctx: ToolContext, input: Record<string, unknown>): Promise<ToolResult> {
...
async function readFile(ctx: ToolContext, input: Record<string, unknown>): Promise<ToolResult> {
  const file = resolveInside(ctx.root, String(input.path));
  const content = await fs.readFile(file, 'utf8');
  return { ok: true, output: truncate(content) };
}

async function writeFile(ctx: ToolContext, input: Record<string, unknown>): Promise<ToolResult> {
...
async function replaceInFile(ctx: ToolContext, input: Record<string, unknown>): Promise<ToolResult> {
  const rel = String(input.path);
  const file = resolveInside(ctx.root, rel);
  const oldText = String(input.oldText ?? '');
  const newText = String(input.newText ?? '');
  const current = await fs.readFile(file, 'utf8');
  if (!current.includes(oldText)) return { ok: false, output: `oldText not found in ${rel}` };
  const ok = await approval(ctx, `replace_in_file ${rel}`, `Replace:\n${truncate(oldText, 600)}\n--- with ---\n${truncate(newText, 600)}`);
  if (!ok) return { ok: false, output: 'User denied replace_in_file' };
  await fs.writeFile(file, current.replace(oldText, newText), 'utf8');
  return { ok: true, output: `Updated ${rel}` };
}

async function grepTool(ctx: ToolContext, input: Record<string, unknown>): Promise<ToolResult> {
  ...
  for (const f of files) {
    ...
    try {
      ...
      lines.forEach((line, i) => {
        if (hits.length < limit && re.test(line)) hits.push(`${f}:${i + 1}: ${line}`);
      });
    } catch {}
  }
...
async function runShell(ctx: ToolContext, input: Record<string, unknown>): Promise<ToolResult> {
  const command = String(input.command || '');
  const timeoutMs = Number(input.timeoutMs || 120000);
  const ok = await approval(ctx, `run_shell`, command);
  if (!ok) return { ok: false, output: 'User denied run_shell' };
