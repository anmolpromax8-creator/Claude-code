...
export class AnthropicProvider implements Provider {
  ...
  async complete(messages: ChatMessage[], tools: ToolDefinition[]): Promise<ProviderResponse> {
    ...
    for (const part of json.content || []) {
      ...
      if (part.type === 'tool_use') toolCalls.push({ id: part.id, name: part.name, input: part.input || {} });
    }
...
function normalizeMessages(messages: ChatMessage[]): any[] {
  ...
  for (const m of messages) {
    if (m.role === 'tool') out.push({ role: 'user', content: `Tool result from ${m.name}:\n${m.content}` });
  ...
  if (merged.length === 0 || merged[0].role !== 'user') merged.unshift({ role: 'user', content: 'Begin.' });
  return merged;
}
