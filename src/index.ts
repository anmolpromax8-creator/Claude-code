...
async function resolveSlashCommand(cmd: string): Promise<string> {
  if (cmd === '/') return commandPalette(slashCommands);
  const matches = slashCommands.filter(c => c.cmd.startsWith(cmd));
  if (matches.length === 1) {
    console.log(chalk.dim(`↳ ${cmd} → ${matches[0].cmd}`));
    return matches[0].cmd;
  }
  if (matches.length > 1) return commandPalette(matches);
  return cmd;
}

async function commandPalette(commands = slashCommands): Promise<string> {
  return select({
...
async function nvidiaModelCachePath(): Promise<string> {
  return path.join(configDir(), 'nvidia-models-cache.json');
}

async function loadCachedNvidiaModels(): Promise<string[]> {
  try {
    const raw = JSON.parse(await fs.readFile(await nvidiaModelCachePath(), 'utf8'));
    return Array.isArray(raw?.models) ? raw.models.filter((x: unknown) => typeof x === 'string') : [];
  } catch {
    return [];
  }
}

async function saveCachedNvidiaModels(models: string[]): Promise<void> {
